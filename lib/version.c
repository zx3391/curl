...
{
  static char out[250];
  char format[14*3 + 1]; /* room for 14 "%s " and trailing zero */
  char *fmtp;
  const char *src[14];
  char ssl_version[40];
#ifdef HAVE_LIBZ
  ...
  *fmtp = 0;
  ...
  msnprintf(out, sizeof(out), format, src[0], src[1], src[2], src[3], src[4],
            src[5], src[6], src[7], src[8], src[9], src[10], src[11], src[13],
            src[14]);
